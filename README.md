...
仍为 18 core / 19 with rasterizer。operation 与 patch 都拒绝额外 key，patch 必须非空，
且只能包含：
...
artifact，并重新用公开 input schema 校验全部 19 个示例。`pnpm test` 会先执行该 drift
