...
| `tiled_preview_write_tmx` | 原生受限 TMX 写出：把 `.tmj` 序列化为与 Tiled 1.12.2 官方 writer 字节级一致的 `.tmx` 新文件（同目录、no-replace、无需 CLI）；自定义属性支持 string/int/float/bool/color/file（官方 writeProperties 字节序），object/class 型属性与其余 profile 外结构、丢精度浮点 fail closed |
