...
| **M2：格式与事务扩展** | **已实现第一、二步：压缩/base64 tile data 只读 + infinite/chunk 地图只读**（summary/region/usage/native preview 可读 chunked 存储：绝对坐标含负值、chunk 外为空、重叠 chunk fail closed、单层 ≤4,096 chunk、chunk data 沿用 layer 级编码解码、preview 需显式 region；全部编辑路径继续 fail closed）——`tiled_get_region`/`tiled_render_preview`/`tiled_analyze_usage`/`tiled_render_map` 按 Tiled 1.12.2 读取器语义解码 base64 + gzip/zlib/zstd（严格 canonical base64、解压输出钉死 width×height×4、LE uint32、单层 64 MiB 上限），全部编辑路径继续 fail closed，`tileDataReadCapabilities` 公布策略。其余仍待实现：无限地图编辑与原 chunk 边界保持、压缩数据写入/转码、内嵌/collection tileset、跨文件可恢复事务、对象模板、复杂属性（含嵌套 class/list）、选择句柄和更多渲染方向 | 覆盖新增 fixture 的字节/语义往返；跨文件故障注入后可自动恢复到提交前或提交后的一致状态 |
