...
详情、只读
校验、增量创建地图、tileset 创建预览、文件删除预览、空图层创建预览、map edits
预览、外部 tileset 挂载预览、
per-tile 元数据更新预览、跨文件原子事务预览与 change
set 提交；探测到 `tmxrasterizer` 时再注册第 30 个高保真地图 PNG 工具。另注册两个不依赖
...
aliases。双 profile 的完整 discovery artifact、包含当前 104 个 v1 application code 的
...
3. handler 已接收到合法输入后发生的领域/应用错误使用稳定 `code`，设置 `isError: true`，
   ...
   `structuredContent.result.error.code`。当前 v1 application registry 有 104 个 code，
   机器 artifact 为
...
保持 29 core / 30 with rasterizer。operation 与 patch 都是 strict object；patch 至少
包含一个字段且只允许：
...
registry 保持 29 core / 30 with rasterizer。语义按 Tiled 1.12.2 官方源码逐条核实：
...
#### 3.2.1 `tiled_preview_transaction`：原子多文件事务（已实现）
...
`targetKind: replace|create|delete`、路径与 revision pin；delete 成员标记
destructive），`expectedRevision` 是有序目标 pin 集的聚合 SHA-256——多文件提案
没有单一当前 revision，聚合 digest 沿用 batch prune 先例。preview 同时把每个成
员标记为该事务所有：成员单独 `tiled_apply_change_set` 返回
`CHANGE_SET_OWNED`，事务过期或完成后释放。pending 事务同时最多 4 个。

apply 仍走 `tiled_apply_change_set`：每个成员计划按当前项目状态重放为精确目标
字节，全部 revision pin 在 canonical 路径全序加锁下复核，然后经
`.tiledmcp/transactions/` 下的崩溃可恢复 redo journal 提交——提交点之前崩溃启动
对账回滚、之后前滚，全部目标落盘或全不落盘；崩溃窗口内被外部写者改动的单个目标
降级为披露 conflict，其余照常前滚。每个目标仍有各自的 before-state checkpoint，
成员变更保持可单独恢复。结果是判别的
`{kind:"transaction", changeSetId, transactionId, results:[...]}` 分支，
`results` 逐成员采用其单独 apply 时的完全相同 wire 形状（replace/create 为
document commit 形状、delete 为 `fileDelete` 分支），成员 change set 的重放返回
对应成员结果而非二次提交。成员在事务批准后被篡改、过期或提前 apply 都会使事务
apply fail closed。staged 内容总量 ≤ 64 MiB。策略字符串冻结于
...
registry 仍为 29 个 core / 30 个含 rasterizer。`layerId` 必须是正整数，并递归定位一个
...
它不额外注册 `tiled_move_layer`，所以 registry 仍是 29 个 core / 30 个含
rasterizer。
...
`tiled_duplicate_layer` standalone tool；registry 因此仍是 29 core / 30 with
rasterizer。
...
| `tiled_select` | **已实现第一刀（无状态变体）**：按谓词（tile 集合按 tileset+localId 匹配忽略 flip 位 / 空 / 非空 / 魔棒——种子四向泛洪同 baseGid 连通区、空种子泛洪空区、限制于 region、结果披露 seedBaseGid）扫描一个有界 tile-layer 区域，把选区作为**纯数据**返回——精确计数、紧包围盒、≤2048 坐标采样（截断披露）；不发 selectionId、不留服务端选区状态，调用方显式把结果喂给 region/cell 工具；四种投影全支持；多边形已补齐（`polygon` 谓词：3..64 像素坐标点、cell 中心偶奇 crossing 判定、半开边规则）；选区组合已补齐（`compose` 谓词：从空集起按序 fold ≤8 步 union/intersect/subtract、步内为不嵌套的基础谓词；`sampleLimit` 1..10,000 对齐 setTiles cell 预算——选区全量作为纯数据直喂编辑，仍无 selectionId 无服务端状态）；`Region.kind="selection"` 引用机制以此设计取代 | `mapPath`, `layerId`, `region?`, `match` |
...
`tiled_stamp_pattern` 工具，因此 registry 保持 29 core / 30 with rasterizer。
...
`tiled_flood_fill` 工具，因此 registry 仍为 29 core / 30 with rasterizer。它只接受
...
`tiled_copy_region` standalone tool，因此 registry 仍为 29 core / 30 with rasterizer。
...
| `tiled_scatter` | **已实现为 `tiled_preview_scatter`**：确定性密度散布——无状态坐标 hash 逐格对 density 判定（同 seed 同输出、平移稳定，绝不 Math.random），第二个加盐 hash 从加权 choice 列表选 tile（null 选择为擦除）；`skipOccupied` 跳过已占用格；产出普通 `mapEdit` change set（精确 `setTiles`），零命中 fail closed；最小间距/避让列表/模板对象散布留后续 | `mapPath`, `layerId`, `region`, `seed`, `density`, `choices`, `skipOccupied?`, `expectedMapRevision`, `expectedDependencyRevisions` |
...
| `tiled_save_prefab` / `tiled_place_prefab` | **已实现等价能力为 `tiled_preview_prefab`**：不落中间 prefab 文件——直接把源地图一块区域（一个 tile 图层的 tile + 可选一个 object 图层中锚定在区域像素范围内的对象）在规划期物化为普通 `setTiles`+`createObject` 操作盖到目标位置，plan 本身即冻结的 prefab（apply 不回读源；可选 `expectedSourceRevision` 规划期断言源 revision）。tile 以 tileset+localId 引用携带（目标缺 tileset fail closed）；空源格默认跳过、`copyEmpty` 时整矩形照搬为擦除；draft profile 之外的对象（自定义属性、模板实例、未知成员）与跨地图 tile 尺寸不一致 fail closed。多图层与水平翻转已补齐（`extraTileLayers` 显式层对列表 ≤16、`flipHorizontal` 对照官方 TileLayer::flip——反对角 cell 翻 V 位其余翻 H 位、仅 tile 层且与 objects 同用 fail closed）；按名匹配/自动建层/持久化 prefab 库留后续 | `mapPath`, `sourceMapPath`, `source`, `target`, `objects?`, `copyEmpty?`, `expectedMapRevision`, `expectedDependencyRevisions`, `expectedSourceRevision?` |
...
registry 为 29 core / 30 with rasterizer。
...
| `tiled_update_tile` | **已实现专用 preview 工具**：批量更新单个已引用 external TSJ（atlas 或 image-collection）的 per-tile probability/class/动画/标量自定义属性/碰撞形状元数据，走独立 `tilesetEdit` change set（哈希域 v2、plan version 2）。image-collection 另接受独占结构 update：`createCollectionTile` 从项目图片新增稀疏条目（planner 读图安全检查、pin 实际像素尺寸并注入 plan，replay 重读比对 fail closed；`tilecount` 与最大 tile 尺寸按 `Tileset::addTile`/`updateTileSize` 语义联动），`removeCollectionTile`（preview 标记 destructive）仅在当前 map 无该 local id 引用（复用 GID 扫描）、无其他项目资产引用该 TSJ（GID span 收缩不得悬空外部引用，复用 fileDelete 引用扫描）且非最后一个条目时删除条目并重算根尺寸 | `mapPath`, `tilesetAssetId`, `expectedMapRevision`, `expectedTilesetRevision`, `updates: [{tileId, patch \| createCollectionTile \| removeCollectionTile}]` |
...
`tiled_remove_tileset_from_map`，因此 registry 保持 29 core / 30 with rasterizer。
...
| `tiled_paint_terrain` | **已实现为可选工具 `tiled_preview_terrain`**：经受控 one-shot `tiled --evaluate` 调用官方 `TileLayer.wangEdit()`（与编辑器 Terrain Brush 同一匹配器）——服务端静态内置脚本、参数以 JSON 字面量内嵌（无代码注入面）、CLI 只写 staging 副本；服务端 diff 目标层后返回**普通 `mapEdit` change set**（精确 `setTiles` cell 写入），apply 不重跑 CLI、未触碰片段逐字保留、可入事务。角坐标寻址角网格（0..width/0..height）、1-based 颜色索引、限 corner/mixed 集与外部 atlas；无变化 fail closed。注：官方 AutoMapping 在 Tiled 1.12.2 无头 evaluate 下不可行（`autoMap` 拒绝 detached map、`tiled.open` 报 Editor not available），已实测确认并放弃该路径 | `mapPath`, `layerId`, `tilesetAssetId`, `wangSetIndex`, `corners`, `expectedMapRevision`, `expectedDependencyRevisions` |
...
| `tiled_register_tile_names` | **读侧已实现为 `tiled_list_tile_names`**：读取服务端自有 `.tiledmcp/tile-names.json`（受限小写标识符 ≤4096 条、逐 tileset 存在性校验并 pin revision、缺文件读作空、弱元数据——localId 逐字披露不复核 tileset 内容）；注册写侧同样已可用：`tiled_preview_tile_names` 走专用 `tileNameEdit` change set——pin 注册表文件 revision（缺失以 null pin）、upsert 校验 .tsj 存在（apply 复核）、删除未注册名 fail closed、≤64 操作/≤4096 名、apply 重放并对批准内容 hash 校验后规范化重写（排序 key）；注册名已可在 `tiled_preview_shape`/`tiled_preview_generate`/`tiled_preview_scatter`/`tiled_select` 的 tile 位以 `{"name": ...}` 替代 TileRef——服务端经注册表+目标 map binding 解析（tileset 未挂 map 或 localId 越界 fail closed） | 无参数 |
...
| `tiled_import_from_image` | **已实现为 `tiled_preview_import_image`**：参考图重采样到目标格网（每 cell 对其像素块做 alpha 加权平均）、按平方 RGB 距离映射最近 palette 色（并列取 palette 先序）为 tile（支持 `{name}` 语义引用、null 色为擦除、全透明块跳过），产出普通 `mapEdit` change set；纯整数算术确定性、全透明区域 fail closed、图片经 safeImage 有界解码 | `mapPath`, `layerId`, `imagePath`, `region`, `palette`, `expectedMapRevision`, `expectedDependencyRevisions` |
...
| `tiled://application-errors` | `application/json` | **已实现。** 当前 104 个 v1 application code，以及 wire location、`INTERNAL_ERROR` fallback、兼容策略和排除边界；内容与提交的 machine artifact 相同 |
...
| **M2：格式与事务扩展** | **已实现第一至四步：压缩/base64 tile data 读取与写回 + infinite/chunk 地图只读 + 跨文件可恢复事务（redo journal 存储核心 + `tiled_preview_transaction` wire 组合层，含每步崩溃注入与启动对账验证；含 create+attach 耦合放行：attach 以 pending create 计划的 prospective 内容预 pin，事务 prepare 用重放内容顶替磁盘依赖）**（encoded 有限 layer 可编辑：实际被写的 layer 按原 encoding/compression 重编码、绝不隐式转码，显式转码走独占 `transcodeTileLayer` operation（csv ↔ base64+""/gzip/zlib/zstd，同目标为 no-op），未触碰与净 no-op layer 保持原 bytes，resize 仍 fail closed）（summary/region/usage/native preview 可读 chunked 存储：绝对坐标含负值、chunk 外为空、重叠 chunk fail closed、单层 ≤4,096 chunk、chunk data 沿用 layer 级编码解码、preview 需显式 region；chunked layer 已支持除 resize 外的全部 tile 操作（floodFill 以已用 chunk 并集为界、种子出界为空填充，对齐编辑器语义；无 region 的 replaceTiles 只扫描存储的非零 cell）：写回按 Tiled 1.12.2 规范化保存形态重分桶、丢空 chunk、(y,x) 排序、bounds 重算，逐 chunk 按层级编码重编码绝不转码，其余 tile 操作与 resize 对 chunked 继续 fail closed）——`tiled_get_region`/`tiled_render_preview`/`tiled_analyze_usage`/`tiled_render_map` 按 Tiled 1.12.2 读取器语义解码 base64 + gzip/zlib/zstd（严格 canonical base64、解压输出钉死 width×height×4、LE uint32、单层 64 MiB 上限），全部编辑路径继续 fail closed，`tileDataReadCapabilities` 公布策略。其余仍待实现：（内嵌 per-tile metadata 编辑已可用：`tiled_update_tile` 以 `embeddedIndex` 寻址、map revision 唯一 pin、source patch 重定位 `tilesets[i]`、`embeddedTilesetEdit` change set 可入事务；结构编辑对内嵌天然不可达）（collection 的读取/渲染/元数据编辑与条目创建/删除已全部可用：binding 报 `collection:true`、稀疏 id fail closed、详情页图片逐个验证并 pin revision、subrect 与 Wang fail closed；条目结构编辑见 `tiled_update_tile`——创建从验证图片注入尺寸、删除需证明无引用且非最后条目）（内嵌只读核心与 tile layer 渲染已可用：summary 单列内嵌 atlas 条目、region 返回 `{kind:"embedded", sourceIndex}` 只读引用、`tiled_get_tileset` 以 `embeddedIndex` 寻址且由 map revision pin、`tiled_render_preview` 绘制内嵌 atlas tile layer——图片相对 map 解析、source 条目报 `{embedded:{sourceIndex}}`；内嵌 collection、pre-1.5 `terrains` 与内嵌 tile object 渲染 fail closed，`embeddedTilesetCapabilities` 公布策略）、tile/XML/嵌套模板与模板属性合并（非 tile JSON 模板的 get_object 读取展开已可用：syncWithTemplate 精确合并、template 块 pin revision）、class 成员引入与 list 元素追加/删除（既有成员/元素的覆写已可用：setClassMembers 保持 JSON 类型、缺失成员 fail closed；setListElements 就地覆写既有 list 元素标量值、保持 JSON 类型与 Tiled type 注解，enum 包装与嵌套元素 fail closed；读取为有界 raw JSON 投影 + valueSemantics 披露）、等距地图的程序化 planner 放行（标准 mapEdit 主链路编辑已可用：planEdits 与 apply 放行 isometric——cell 存储与对象像素坐标方向无关，summary 的 editableProfile 报 `isometric-tmj-editable-core`；summary/region/usage 只读经 `allowIsometric` 内部 opt-in；shape/generate/scatter/prefab/terrain 全部程序化 planner 已放行 isometric（terrain 经真 CLI 铁证：wangEdit 于等距 staging 地图 setCorner 行为与正交一致）；staggered/hexagonal 已降级为 summary/region/usage 只读——`allowStaggeredHexagonal` 内部 opt-in、summary 披露 staggeraxis/staggerindex/hexsidelength（Tiled writer 对这两种投影恒写，缺失 fail closed）、hexagonal region 的 flip 位按官方 rotate60/rotate120 语义解码、profile 报 `staggered-hexagonal-tmj-read-only`、编辑维持 fail closed、渲染已可用（`tiled_render_hexagonal`：官方 HexagonalRenderer 变换、staggered=hexSideLength 0 退化、(screenY,screenX) 序等价官方两轴行序、hex 旋转位 fail closed、profile 同等距渲染器），oblique 全面拒绝；渲染已可用：`tiled_render_isometric` 按官方 IsometricRenderer 变换（top corner=(x−y)·tw/2+originX、image 底左锚定于 (top−tw/2, top+th)、对角线扫描序）把区域画成自身菱形 PNG，严格 profile——外部 atlas 且 tile 尺寸=网格、无 collection/透明色/反对角翻转/image/group 层，object 层跳过并披露 id）、TMX region/详情读取（TMX map summary 只读、raw-gid region 读取与 fileDelete XML 引用扫描已可用：自研有界 fail closed XML 子集解析器，拒 DOCTYPE/实体/PI/CDATA，零依赖零 XXE 面；TMX 写出已可用：`tiled_preview_write_tmx` 原生把受限 profile `.tmj` 序列化为与 Tiled 1.12.2 官方 writer 字节级一致的 `.tmx`（金标准对拍验证：QXmlStreamWriter 缩进 1、QString::number %g 六位有效数字浮点、mapwriter.cpp 属性序、QXml 转义集），gid/firstgid/tileset 引用逐字保留故目标限源同目录、no-replace；自定义属性放行 string/int/float/bool/color/file 六型（对照官方 writeProperties：name 字典序、string 隐式、换行字符串走元素文本、float %g，金标准对拍；object/class 型 fail closed；prefab 对象盖章的标量 properties 已放行——createObject+updateObject({properties:{set}}) 两连 op、新 id 由 nextobjectid 预测且 revision pin 保证稳定；object 型属性 XML 写出已放行（type="object" value=id，金标准对拍）；TSX tileset 级标量 properties 已放行（image 前，金标准对拍）；class 型已经可选 `projectFilePath` 无损写出——TMX/TSX/TX 三工具接受 project 文件，成员类型由 propertyTypes class 定义解析（仅 value 中出现的成员、name 字典序、嵌套 class 递归 ≤8 层、enum 注解成员 fail closed），plan pin project revision 且 apply 复核后按同一定义重放（`tiled --project` 金标准对拍：propertytype 属性 + 嵌套 properties 字节一致）；无 project 时维持 fail closed（真 CLI 铁证：无上下文官方 exporter 丢 propertytype 且成员类型漂移 qlonglong））；内嵌 tileset、image/group 层、模板实例、未知成员与丢精度浮点全部 fail closed；产出 producer=native 的 `fileExport` change set，apply 在 pin 的源 revision 下原生重序列化并 hash 校验，无需 CLI；TSX 写出同样已可用：`tiled_preview_write_tsx` 把受限 profile `.tsj` atlas 序列化为字节级一致 `.tsx`——声明网格必须可由图片尺寸+margin/spacing 推导（官方 exporter 会重算，不一致 fail closed 而非漂移），per-tile 元数据/wangsets/属性/未知成员 fail closed；`.tx` 模板写出同样已可用：`tiled_preview_write_tx` 对照 writeObjectTemplate——裸 `<template>` 根、基对象按 isTemplateBase 不写 id/x/y、复用已金标准验证的 object writer，tile/嵌套模板 fail closed，exportKind=template）、选择句柄和更多渲染方向 | 覆盖新增 fixture 的字节/语义往返；跨文件故障注入后可自动恢复到提交前或提交后的一致状态 |
