...
| `tiled_register_tile_names` | **读侧已实现为 `tiled_list_tile_names`**：读取服务端自有 `.tiledmcp/tile-names.json`（受限小写标识符 ≤4096 条、逐 tileset 存在性校验并 pin revision、缺文件读作空、弱元数据——localId 逐字披露不复核 tileset 内容）；注册写侧（`tiled_preview_tile_names` upsert/delete 专用 change set）留后续 | 无参数 |
