...
| `tiled_save_prefab` / `tiled_place_prefab` | **已实现等价能力为 `tiled_preview_prefab`**：不落中间 prefab 文件——直接把源地图一块区域（一个 tile 图层的 tile + 可选一个 object 图层中锚定在区域像素范围内的对象）在规划期物化为普通 `setTiles`+`createObject` 操作盖到目标位置，plan 本身即冻结的 prefab（apply 不回读源；可选 `expectedSourceRevision` 规划期断言源 revision）。tile 以 tileset+localId 引用携带（目标缺 tileset fail closed）；空源格默认跳过、`copyEmpty` 时整矩形照搬为擦除；draft profile 之外的对象（自定义属性、模板实例、未知成员）与跨地图 tile 尺寸不一致 fail closed。多图层按名匹配/自动建层/翻转/持久化 prefab 库留后续 | `mapPath`, `sourceMapPath`, `source`, `target`, `objects?`, `copyEmpty?`, `expectedMapRevision`, `expectedDependencyRevisions`, `expectedSourceRevision?` |
