...
- native v1 支持静态 external atlas、透明色、layer opacity、orthogonal H/V/D 与 bit 29
  ...
  非有限/非正交地图不做近似；image-collection tile 仅在与 map grid 完全同尺寸
  时绘制（cell 与 tile object），逐 tile 图片计入 atlas 源预算，其余尺寸 fail
  closed。
