...
| **M2：格式与事务扩展** | **已实现第一至四步：压缩/base64 tile data 读取与写回 + infinite/chunk 地图只读 + 跨文件可恢复事务（redo journal 存储核心 + `tiled_preview_transaction` wire 组合层，含每步崩溃注入与启动对账验证；含 create+attach 耦合放行：attach 以 pending create 计划的 prospective 内容预 pin，事务 prepare 用重放内容顶替磁盘依赖）**（encoded 有限 layer 可编辑：实际被写的 layer 按原 encoding/compression 重编码、绝不隐式转码，显式转码走独占 `transcodeTileLayer` operation（csv ↔ base64+""/gzip/zlib/zstd，同目标为 no-op），未触碰与净 no-op layer 保持原 bytes，resize 仍 fail closed）（summary/region/usage/native preview 可读 chunked 存储：绝对坐标含负值、chunk 外为空、重叠 chunk fail closed、单层 ≤4,096 chunk、chunk data 沿用 layer 级编码解码、preview 需显式 region；chunked layer 的 setTiles/stampPattern/floodFill/转码已可用（floodFill 以已用 chunk 并集为界、种子出界为空填充，对齐编辑器语义）：写回按 Tiled 1.12.2 规范化保存形态重分桶、丢空 chunk、(y,x) 排序、bounds 重算，逐 chunk 按层级编码重编码绝不转码，其余 tile 操作与 resize 对 chunked 继续 fail closed）——`tiled_get_region`/`tiled_render_preview`/`tiled_analyze_usage`/`tiled_render_map` 按 Tiled 1.12.2 读取器语义解码 base64 + gzip/zlib/zstd（严格 canonical base64、解压输出钉死 width×height×4、LE uint32、单层 64 MiB 上限），全部编辑路径继续 fail closed，`tileDataReadCapabilities` 公布策略。其余仍待实现：chunked layer 的 copyRegion/replaceTiles、collection tileset 的 sheet 渲染、地图 preview 渲染与编辑，以及内嵌 tileset（collection 的 summary/region/object/详情/检索读取与显式 tile 选集渲染已可用：binding 报 `collection:true`、稀疏 id fail closed、详情页图片逐个验证并 pin revision、subrect 与 Wang fail closed）、对象模板、复杂属性（含嵌套 class/list）、选择句柄和更多渲染方向 | 覆盖新增 fixture 的字节/语义往返；跨文件故障注入后可自动恢复到提交前或提交后的一致状态 |
