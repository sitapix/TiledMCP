...
| `tiled_register_tile_names` | **读侧已实现为 `tiled_list_tile_names`**：读取服务端自有 `.tiledmcp/tile-names.json`（受限小写标识符 ≤4096 条、逐 tileset 存在性校验并 pin revision、缺文件读作空、弱元数据——localId 逐字披露不复核 tileset 内容）；注册写侧同样已可用：`tiled_preview_tile_names` 走专用 `tileNameEdit` change set——pin 注册表文件 revision（缺失以 null pin）、upsert 校验 .tsj 存在（apply 复核）、删除未注册名 fail closed、≤64 操作/≤4096 名、apply 重放并对批准内容 hash 校验后规范化重写（排序 key） | 无参数 |
